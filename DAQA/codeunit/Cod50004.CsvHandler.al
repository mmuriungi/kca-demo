codeunit 50104 "DAQA Csv Handler"
{
    ...
    begin
        ...
        //Add formula column -- Hii ni kwa survvey tu
        if SheetSequence = 1 then
            ExcelBuffer.AddColumn('Choices Present', false, '', false, false, false, '', ExcelBuffer."Cell Type"::Text);
        ...
        if not RecRef.IsEmpty and not RecRef.IsDirty then begin
            if RecRef.FindSet() then begin
                repeat
                    ...
                    //Add formula column -- Hii ni kwa survvey tu
                    FormulaReferenceSheetName := '''' + 'Drill Down Answers' + '''';
                    if SheetSequence = 1 then
                        ExcelBuffer.addcolumn('=IF(COUNTIFS(' + FormulaReferenceSheetName + '!$D:$D,C2,' + FormulaReferenceSheetName + '!$E:$E,"<>")>0,"Yes","No")', true, '', false, false, false, '', ExcelBuffer."Cell Type"::Text);
                until RecRef.Next() = 0;
            end;
        end;
    ...
    procedure importFromExcel(ArrRecref: array[20] of RecordRef; ArrSheetName: array[20] of Text; arraylength: Integer; FieldsDict: Dictionary of [Integer, List of [Integer]])
    var
        i: Integer;
    ...
    begin
        ...
        for i := 1 to arraylength do begin
            ...
            ImportRecordRef(ArrRecref[i], ExcelBuffer, FieldsList);
            ;
        end;
    end;

    procedure ImportRecordRef(var RecRef: RecordRef; var ExcelBuffer: Record "Excel Buffer" temporary; fields: List of [Integer])
    var
        field: Integer;
        RowNo: Integer;
        MaxRowNo: Integer;
        ...
        fieldValue: Text;
        col: Integer;
        QuizType: Enum "Question Answer Type";
        QuizCate: Enum "Question Category";
        boolValue: Boolean;
    begin
    ...
    procedure GetQuiz(SurveyCode: Code[20]; var Quiz: Record "Project Monitor Quiz")
    begin
        Quiz.SetRange("Survey Code", SurveyCode);
        Quiz.FindSet();
    end;

    procedure GetAnswers(SurveyCode: Code[20]; var Answers: Record "Project Quiz Answers")
    begin
        Answers.SetRange("Survey Code", SurveyCode);
        Answers.FindSet();
    end;

    local procedure GetAnswerValue(Answers: Record "Project Quiz Answers"): Text
    begin
        case Answers."Question Type" of
            Answers."Question Type"::Text:
                exit(Answers."Text Answer");
            Answers."Question Type"::Number:
                exit(Format(Answers."Number Answer"));
            Answers."Question Type"::Date:
                exit(Format(Answers."Text Answer"));
            Answers."Question Type"::"Single Choice",
            Answers."Question Type"::"Multiple Choice":
                exit(Answers."Text Answer");
            Answers."Question Type"::Rating:
                exit(Format(Answers."Number Answer"));
            Answers."Question Type"::"Yes/No":
                exit(Format(Answers."Boolean Answer"));
            else
                exit('');
        end;
    end;

    procedure ExportQuestionsAndAnswers(var ExcelBuffer: Record "Excel Buffer" temporary; SheetName: Text; SurveyCode: Code[20])
    var
        Questions: Record "Project Monitor Quiz";
        Answers: Record "Project Quiz Answers";
        QuestionList: List of [Integer];
        AnsweredByList: List of [Text];
        AnsweredBy: Text;
        QuestionNo: Integer;
        RowNo: Integer;
        ColNo: Integer;
        AnswerText: Text;
    begin
        // Helper function to get answer based on question type


        // Get unique list of questions
        Questions.SetRange("Survey Code", SurveyCode);
        Questions.SetRange("Question Category", Questions."Question Category"::Question);
        if Questions.FindSet() then
            repeat
                if not QuestionList.Contains(Questions."Quiz No.") then
                    QuestionList.Add(Questions."Quiz No.");
            until Questions.Next() = 0;

        // Get unique list of respondents (emails)
        Answers.SetRange("Survey Code", SurveyCode);
        if Answers.FindSet() then
            repeat
                if not AnsweredByList.Contains(Answers."Answered By Email") then
                    AnsweredByList.Add(Answers."Answered By Email");
        ...
        ExcelBuffer.NewRow();
        ExcelBuffer.AddColumn('Answered By', false, '', false, false, false, '', ExcelBuffer."Cell Type"::Text);

        ColNo := 2;
        foreach QuestionNo in QuestionList do begin
            Questions.Reset();
            Questions.SetRange("Survey Code", SurveyCode);
            Questions.SetRange("Quiz No.", QuestionNo);
            Questions.SetRange("Question Category", Questions."Question Category"::Question);
            if Questions.FindFirst() then begin
                ExcelBuffer.AddColumn(Questions.Question, false, '', false, false, false, '', ExcelBuffer."Cell Type"::Text);
                ColNo += 1;
            end;
        end;

        // Add answers for each respondent
        RowNo := 2;
        foreach AnsweredBy in AnsweredByList do begin
            ExcelBuffer.NewRow();
            ExcelBuffer.AddColumn(AnsweredBy, false, '', false, false, false, '', ExcelBuffer."Cell Type"::Text);

            ColNo := 2;
            foreach QuestionNo in QuestionList do begin
                Answers.SetRange("Survey Code", SurveyCode);
                Answers.SetRange("Quiz No.", QuestionNo);
                Answers.SetRange("Answered By Email", AnsweredBy);
                if Answers.FindFirst() then begin
                    AnswerText := GetAnswerValue(Answers);
                    ExcelBuffer.AddColumn(AnswerText, false, '', false, false, false, '', ExcelBuffer."Cell Type"::Text);
                end else
