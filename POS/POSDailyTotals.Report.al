...
Report 99402 "POS Daily Totals"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/POS Daily Totals.rdlc';
    ...
    dataset
    {
        dataitem(posHeader; "POS Sales Header")
        {
            ...
            column(BankAccountValue; BankAccounts.Name)
            {
            }
            column(Cashier; posHeader.Cashier)
            {
            }
            ...
            begin
                ...
                Clear(BankAccountValue);
                if posHeader."Payment Method" = posHeader."payment method"::Cash then
                    BankAccountValue := posHeader."Cash Account"
                else
                    BankAccountValue := posHeader."Bank Account";
                Clear(BankAccounts);
                BankAccounts.Reset;
                BankAccounts.SetRange("No.", BankAccountValue);
                if BankAccounts.Find('-') then begin
                end else begin
                    BankAccounts.Name := BankAccountValue;
                end;
            end;
