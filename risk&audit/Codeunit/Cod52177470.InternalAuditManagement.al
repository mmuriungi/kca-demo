codeunit 52177470 "Internal Audit Management"
{
    ...
    var
        ...
        Text0003: Label 'Do you want to send the Notification %1 to %2?';
    ...
    begin
        ...
        IF Communication.Sent THEN
            ERROR('Communication has been done already!!');
    ...
    begin
        ...
        IF Communication.Status = Communication.Status::Sent THEN
            ERROR('Communication has already been sent!!');

        IF NOT Communication."Notify Department" THEN BEGIN
            //Receipients.Add(Communication."Receipient Name")
            Receipients := Communication."Receipient Name"
        END ELSE
            //Receipients.Add(Communication."Shortcut Dimension 2 Code");
            Receipients := Communication."Shortcut Dimension 2 Code";

        IF NOT CONFIRM(Text0003, FALSE, Communication."No.", Receipients) THEN
            EXIT
        ELSE BEGIN
            CASE Communication."Communication Type" OF
                Communication."Communication Type"::"E-Mail", Communication."Communication Type"::"E-Mail & SMS":
    ...
    begin
        ...
        IF NOT CONFIRM(Text0005, TRUE, GetDimensionValue(AuditHeader."Shortcut Dimension 2 Code")) THEN
            EXIT
        ELSE BEGIN
    ...
    procedure GetDimensionValue(DimCode: Code[50]): Text[250]
    begin
        DimVal.RESET;
    ...
    procedure GetEmployeeName(UserName: Code[50]): Text[250]
    begin
    ...
    begin
        ...
        if Incident."Screen Shot".HasValue then
            AuditSetup.TestField("Attachment Path");

        IF NOT CONFIRM(Text0001, FALSE, Incident."Incident Reference") THEN
            EXIT
        ELSE BEGIN
    ...
    var
        ...
        EmailBody: Label 'Dear <Stong>%1</Strong>, <br><br> Kindly find attached the Audit Report that was conducted on your Department for the Period %2. <br><br>Thank you. <br><br>Regards, <br><br>%3';
        AuditLines: Record "Audit Lines";
    begin
        AuditSetup.GET;
        CompanyInfo.GET;

        IF NOT CONFIRM(ConfirmSendAuditReport, TRUE, AuditHeader."No.") THEN
            EXIT
        ELSE BEGIN
            IF NOT AuditHeader."Notification Sent" THEN BEGIN
                ...
                REPORT.SAVEASPDF(Report::"Internal Audit Report", FileName, AuditHead2);
            END;
    ...
    begin
        ...
        IF NOT CONFIRM(ConfirmSurveySendMssg, TRUE, Audit."No.") THEN
            EXIT
        ELSE BEGIN
            IF NOT Audit."Notification Sent" THEN BEGIN
                ...
                REPORT.SAVEASPDF(Report::"Risk Survey", FileName, Audit2);
            END;

            //Get Risk Champions
            Champions.RESET;
            Champions.SETRANGE(Type, Champions.Type::"Risk Manager");
    ...
    begin
        ...
        IF AuditLines.FIND('-') THEN BEGIN
            CASE AuditLines."Audit Line Type" OF
                AuditLines."Audit Line Type"::"Report Recommendation":
                    BEGIN
                        REPEAT
                            AuditSetup.GET;
                            IF CALCDATE(AuditSetup."Report Deadline Reminder", AuditLines.Date) = TODAY THEN
                                //Get Employee
    ...
    var
        AuditHeader: Record "Audit Header";
    begin
        IF AuditHeader.GET(DocNo) THEN
            AuditHeader."Reviewed By" := UserName;
    end;
    ...
    var
        ...
        UserSetup: Record "User Setup";
    begin
        CASE AuditHeader.Type OF
            AuditHeader.Type::"Audit Report":
                BEGIN
                    AuditLines.RESET;
                    AuditLines.SETRANGE("Document No.", AuditHeader."No.");
                    IF AuditLines.FIND('-') THEN BEGIN
    ...
    begin
        ...
        IF NOT CONFIRM(SentReportMssg, FALSE, AuditHeader."No.") THEN
            EXIT
        ELSE BEGIN
            /*
            IF NOT AuditHeader."User Reviewed" THEN BEGIN
                IF UserSetup.GET(AuditHeader.Auditee) THEN BEGIN
                    ...
                    TimeNow := FORMAT(TIME);
                    SMTP.CreateMessage(SenderName, SenderAddress, Receipient, Subject, '');
                    SMTP.AppendBody(STRSUBSTNO(SentReportEmail, 'Sir/Madam', AuditHeader."No.", SenderName, SenderName));
                    SMTP.AddAttachment(FileName, Attachment);
                    SMTP.Send;
                END;
    ...
    begin
        if not Confirm(ConfirmSendRisk, true, RiskHeader."No.", RiskHeader."Employee No.") then
            ...
            SenderName := CompanyInfo.Name;
            SenderAddress := CompanyInfo."E-Mail";
            Receipient := GetEmployeeEmailFromUserID(RiskHeader."Created By");
            TimeNow := FORMAT(TIME);
    ...
    begin
        if not Confirm(ConfirmSendRisk, false, Incident."Incident Reference", Incident."Employee No") then
            ...
            Receipient := GetEmployeeEmailFromUserID(Incident.User);
            TimeNow := FORMAT(TIME);
    ...
    begin
        if not Confirm(ConfirmSendRisk, true, RiskHeader."No.", RiskHeader."Risk Region Name") then
            ...
            Receipient := GetRiskChampionsDeptEmails(RiskHeader."Risk Region");
    ...
    begin
        if not Confirm(ConfirmSendRisk, true, RiskHeader."No.", RiskHeader."Risk Region Name") then
            ...
            Receipient := GetRiskChampions2DeptEmails(RiskHeader."Risk Region");
            //ReceipientCC.Add(GetRiskOfficersEmails);
            TimeNow := FORMAT(TIME);
            Subject := 'Risk ' + RiskHeader."No." + ' ' + RiskHeader."Risk Region Name";
    ...
    begin
        IF NOT CONFIRM(ConfirmSendtoOfficer, FALSE, RiskHeader."No.") THEN
            EXIT
        ELSE BEGIN
    ...
    begin
        IF NOT CONFIRM(ConfirmSendtoHOD, FALSE, RiskHeader."No.") THEN
            EXIT
        ELSE BEGIN
            IF CONFIRM(ConfirmSendEmailtoHOD, FALSE)
             THEN BEGIN
                CompanyInfo.GET;
                CompanyInfo.TESTFIELD(Name);
                CompanyInfo.TESTFIELD("E-Mail");
                ...
                Receipient := GetDeptHOD(RiskHeader."Risk Region", RiskHeader."Risk Department");
                //ReceipientCC.Add(GetRiskOfficersEmails);
                ReceipientCC := GetRiskOfficersEmails;
    ...
    begin
        IF NOT CONFIRM(ConfirmSendtoChampion2, FALSE, RiskHeader."No.") THEN
            EXIT
        ELSE BEGIN
            RiskHeader."Document Status" := RiskHeader."Document Status"::"Risk Owner";
            RiskHeader.MODIFY(TRUE);
    ...
    begin
        ...
        IF RiskLine.FIND('-') THEN BEGIN
            REPEAT
                ...
                CASE Type OF
                    Type::Corporate:
                        RiskRegister.Type := RiskRegister.Type::Corporate;
                    Type::Department:
                        RiskRegister.Type := RiskRegister.Type::Department;
                    Type::Project:
                        RiskRegister.Type := RiskRegister.Type::Project;
                END;
    ...
    begin
        IF NOT CONFIRM(ConfirmChange, FALSE) THEN
            EXIT
        ELSE BEGIN
            RiskLine2.RESET;
        ...
        ELSE BEGIN
            ...
            IF RiskLine2.FIND('-') THEN BEGIN
                ...
                IF RiskValueSetup.FIND('-') THEN BEGIN
                    CASE TRUE OF
                        (RiskLine2.Tolerance >= RiskValueSetup."Start Value") AND (RiskLine2.Tolerance <= RiskValueSetup."End Value"):
                            BEGIN
                                /*RiskLine2.RESET;
                                RiskLine2.SETRANGE("Document No.",RiskLine."Document No.");
                                RiskLine2.SETRANGE(Type,RiskLine2.Type::"KRI(s)");
                                ...
                                RiskLine2.RESET;
                                RiskLine2.SETRANGE("Document No.", RiskLine."Document No.");
                                IF RiskLine2.FIND('-') THEN
                                    REPEAT
                                        RiskLine2.Appetite := RiskValueSetup.Value;
                                        RiskLine2.MODIFY;
    ...
    begin
        ...
        ELSE BEGIN
            ...
            SMTP.CreateMessage(SenderName, SenderAddress, Receipient, Subject, '', TRUE);
            SMTP.AppendBody(STRSUBSTNO(AuditeeReportEmail, GetEmployeeName(AuditHeader."Auditee User ID"), AuditHeader."No.", CompanyInfo.Name));
            SMTP.Send;
    ...
    begin
        ...
        ELSE BEGIN
            ...
            Receipient := GetEmployeeEmailFromUserID(AuditHeader."Created By");
            Subject := 'Audit Report';
            TimeNow := FORMAT(TIME);
    ...
    procedure GetEmployeeEmailFromUserID(UserName: Code[50]): Text[100]
    begin
        IF UserSetup.GET(UserName) THEN BEGIN
            Employee.RESET;
    ...
    local procedure GetRiskChampionsDeptEmails(RiskRegion: Code[100]): Text
    var
    ...
    begin
        ...
        if RiskChampions.FindFirst() then begin
            IF Employee.GET(RiskChampions."Escalator ID") THEN
                EXIT(Employee."Company E-Mail");
        end;
    end;
    ...
    begin
        IF NOT CONFIRM(ConfirmSendBackMssg, FALSE, RiskHeader."No.") THEN
            EXIT
        ELSE BEGIN
            RiskHeader."Document Status" := RiskHeader."Document Status"::Champion;
            RiskHeader.MODIFY(TRUE);
    ...
    begin
        IF NOT CONFIRM(ConfirmSendToRegisterMssg, FALSE, RiskHeader."No.") THEN
            EXIT
        ELSE BEGIN
            CASE RiskHeader."Document Status" OF
                RiskHeader."Document Status"::"Risk Owner":
                    DefaultOption := 1;
                RiskHeader."Document Status"::"Risk Manager":
                    DefaultOption := 2;
            END;
            ...
            CASE TRUE OF
                Selection = 1:
                    SendToRiskRegister(RiskHeader, 2);
                Selection = 2:
                    SendToRiskRegister(RiskHeader, 3);
            END;
            RiskHeader."Document Status" := RiskHeader."Document Status"::Champion;
            RiskHeader.MODIFY(TRUE);
        END;
    ...
    begin
        IF NOT CONFIRM(SendToCorpRegConfirm, FALSE, RiskReg."Document No.") THEN
            EXIT
        ELSE BEGIN
            RiskReg2.RESET;
            ...
            IF RiskReg2.FIND('-') THEN BEGIN
                REPEAT
                    RiskReg2.Type := RiskReg2.Type::Corporate;
                    RiskReg2.MODIFY;
    ...
    begin
        IF NOT CONFIRM(ConfirmArchiveRisk, FALSE, RiskRegister."Document No.") THEN
            EXIT
        ELSE BEGIN
            RiskRegister2.RESET;
        ...
        CASE AuditHeader.Type OF
            AuditHeader.Type::"Audit Report":
                BEGIN
                    ...
                    IF AuditLine2.FINDSET THEN
                        AuditLine2.DELETEALL;
    ...
    begin
        CASE AuditHeader.Type OF
            AuditHeader.Type::"Audit Program":
                BEGIN
    ...
    begin
        ...
        ELSE BEGIN
            Project.RESET;
            IF PAGE.RUNMODAL(PAGE::Projects, Project) = ACTION::LookupOK THEN BEGIN
                IF CONFIRM(ConfirmSendEmailtoProjectManager, FALSE)
                   THEN BEGIN
                    ...
                    Receipient := Project."Project Manager E-Mail";
                    //ReceipientCC.Add(GetRiskOfficersEmails);
                    ReceipientCC := GetRiskOfficersEmails;
                    Subject := 'Risk Identified';
                    TimeNow := FORMAT(TIME);
    ...
    begin
        ...
        IF RiskRegister.FINDLAST THEN
            NextEntryNo := RiskRegister."Entry No." + 1
        ELSE
            NextEntryNo := 1;
    end;
    ...
    begin
        ...
        IF AuditRecomm.FINDLAST THEN
            RecomEntryNo := AuditRecomm."Entry No." + 1
        ELSE
            RecomEntryNo := 1;
    end;
        ...
        IF ScopeList.RUNMODAL = ACTION::LookupOK THEN BEGIN
            ...
            IF AuditLineRecCopy.FIND('-') THEN BEGIN
                REPEAT
                    ...
                    IF ((NOT CheckProgScopeExits(AuditLine2."Document No.", AuditLine2."Scope Line No.")) AND
                       (NOT CheckWorkpaperScopeIsNotSelected(AuditHeader."Audit Program No.", AuditLine2."Scope Line No."))) THEN
                        AuditLine2.INSERT;
    ...
    local procedure GetDeptHOD(Dim1: Code[30]; Dim2: Code[30]): Text
    begin
        UserSetup.RESET;
    ...
    local procedure GetScopeLineNo(DocNo: Code[100]): Integer
    var
    ...
    begin
        ...
        IF ALine.FINDLAST THEN
            ScopeLineNo := ALine."Line No." + 1000
        ELSE
            ScopeLineNo := 1000;
    end;

    procedure GetFavObservationNo(DocNo: Code[100]): Integer
    var
        AuditLines: Record "Audit Lines";
    begin
        AuditLines.RESET;
        ...
        IF AuditLines.FINDLAST THEN
            EXIT(AuditLines."Line No." + 10000)
        ELSE
            EXIT(10000);
    end;

    procedure GetUnFavObservationNo(DocNo: Code[100]): Integer
    var
        AuditLines: Record "Audit Lines";
    begin
        AuditLines.RESET;
    ...
    local procedure GetScopeNo(DocNo: Code[50]): Integer
    begin
        GlobalAuditLine.RESET;
        ...
        IF GlobalAuditLine.FINDLAST THEN
            EXIT(GlobalAuditLine."Line No." + 10000)
        ELSE
            EXIT(10000);
    end;
        ...
        IF AuditLineRec.FIND('-') THEN BEGIN
            REPEAT
                ...
                IF LinesRec.FINDSET THEN
                    REPEAT
                        LinesRec."WorkPlan Ref" := AuditRec."No.";
    ...
    procedure GetRiskMELineNo(DocNo: Code[50]): Integer
    var
    ...
    begin
        ...
        IF RiskLineRec.FINDLAST THEN
            EXIT(RiskLineRec."Line No." + 10000)
        ELSE
            EXIT(10000);
    end;

    local procedure CheckMELineNoExist(DocNo: Code[50]; Type: Option " ",KRI,Mitigation; LineNo: Integer): Boolean
    var
        RiskLineRec: Record "Risk Line";
    begin
        RiskLineRec.RESET;
        ...
        IF RiskLineRec.FINDFIRST THEN
            EXIT(TRUE)
        ELSE
            EXIT(FALSE);
    end;

    local procedure CheckProgScopeExits(DocNo: Code[50]; ScopeLineNo: Integer): Boolean
    var
        AuditLines: Record "Audit Lines";
    begin
        AuditLines.RESET;
        ...
        IF AuditLines.FINDFIRST THEN
            EXIT(TRUE)
        ELSE
            EXIT(FALSE);
    end;

    local procedure CheckWorkpaperScopeIsNotSelected(ProgNo: Code[50]; ScopeLineNo: Integer): Boolean
    var
        AuditLineRec: Record "Audit Lines";
    begin
        AuditLineRec.RESET;
        ...
        IF AuditLineRec.FINDFIRST THEN
            IF AuditLineRec."WorkPlan Ref" <> '' THEN
                EXIT(TRUE)
            ELSE
                EXIT(FALSE);
    end;
    ...
    begin
        ...
        IF AuditLines.FINDSET THEN BEGIN
            ...
            REPEAT
                MESSAGE('Prog No - ' + AuditLinesCopy."Document No." + '-' + WorkpaperNo);
    ...
    begin
        IF NOT CONFIRM(SendToDeptRegConfirm, FALSE, RiskReg."Document No.") THEN
            EXIT
        ELSE BEGIN
            RiskReg2.RESET;
            RiskReg2.SETRANGE("Document No.", RiskReg."Document No.");
            IF RiskReg2.FIND('-') THEN BEGIN
                REPEAT
                    RiskReg2.Type := RiskReg2.Type::Department;
                    RiskReg2.MODIFY;
                UNTIL RiskReg2.NEXT = 0;
            END;
            MESSAGE(SentSuccessFully, RiskReg."Document No.");
        ...
        IF RiskLine.FIND('-') THEN BEGIN
            REPEAT
                ...
                IF RiskRegister.FIND('-') THEN BEGIN
                    CASE RiskLine."M & E Type" OF
                        RiskLine."M & E Type"::KRI:
                            BEGIN
                                RiskRegister."KRI(s) Status" := RiskLine."KRI(s) Status";
                            END;
                        RiskLine."M & E Type"::Mitigation:
                            BEGIN
                                RiskRegister."Mitigation Status" := RiskLine."Mitigation Status";
                                RiskRegister."Mitigation Owner" := RiskLine."Mitigation Owner";
                            END;
