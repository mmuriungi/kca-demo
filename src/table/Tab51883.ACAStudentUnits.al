...
table 51883 "ACA-Student Units"
{
    ...
    fields
    {
            ...
            begin
                ...
                */
                ...
                IF UnitsS.FIND('-') THEN BEGIN
                    ...
                    "Special Programme Class" := UnitsS."Special Programme Class";
                    /*
                    CReg.RESET;
                    ...
                    */
                    ...
                    "No. Of Units" := UnitsS."Credit Hours";
        ...
        {
            CalcFormula = Lookup("ACA-Lecturers Units - Old".Lecturer WHERE(Programme = FIELD(Programme),
                                                                             Stage = FIELD(Stage),
                                                                             Unit = FIELD(Unit),
                                                                             Semester = FIELD(Semester)));
            FieldClass = FlowField;
...
#pragma warning restore AL0603
        ...
        field(60195; "Occurances"; Integer)
        {
            Caption = 'Occurances';
            Editable = false;
            FieldClass = FlowField;
    ...
    trigger OnDelete()
    var
        ExamR: Record "ACA-Exam Results";
        ExamR2: Record "ACA-Exam Results";
    ...
    begin
        ...
        ExamR.SETRANGE(ExamR.Semester, Rec.Semester);
        ExamR.SETRANGE(ExamR.Cancelled, FALSE);
        ExamR.SetFilter(ExamR.score, '>0');
        IF ExamR.FIND('-') THEN begin
            //ExamR.DELETEALL;
            ...
            if StudUnits.Find('-') then begin
                if StudUnits.Count < 2 then begin
                    ...
                    ExamR2.RESET;
                    ExamR2.SETRANGE(ExamR2."Student No.", "Student No.");
                    ExamR2.SETRANGE(ExamR2.Unit, Unit);
                    ExamR2.SETRANGE(ExamR2.Semester, Rec.Semester);
                    ExamR2.SETRANGE(ExamR2.Cancelled, FALSE);
                    ExamR2.SETRANGE(ExamR2."Reg. Transaction ID", "Reg. Transacton ID");
                    if ExamR2.find('-') then begin
                        if confirm('Delete Marks associated with the Units?', false) = true then
                            ExamR2.DeleteAll(true);
                    end;
                end;
            end;
        end;
    ...
    begin
        ...
        StudUnits.SETRANGE(StudUnits."Student No.", "Student No.");
        StudUnits.SETRANGE(StudUnits.Unit, Unit);
        StudUnits.SetFilter("Reg. Transacton ID", '<>%1', Rec."Reg. Transacton ID");
        if StudUnits.Find('-') then Error('Unit already exists in: ' + Format(StudUnits.Semester));
        ACASemesters.RESET;
        ACASemesters.SETRANGE(Code, Rec.Semester);
        IF ACASemesters.FIND('-') THEN BEGIN
    ...
    var
        ...
        // ExamR: Record "ACA-Exam Results";
        Studnits: Record "ACA-Student Units";
        // ExamSetup: Record "ACA-Exams Setup";
        StatusC: Record "ACA-Post Grad Change History";
        ACAUnitsSubjects44: Record "ACA-Units/Subjects";
