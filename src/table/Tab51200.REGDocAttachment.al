table 51200 "REG-Doc Attachment"
{
    Caption = 'REG-Doc Attachment';
    ...
    fields
    {
        ...
        field(5; "File Name"; Text[250])
        {
            ...
            trigger OnValidate()
            var
                DocumentAttachmentMgmt: Codeunit "Document Mgmt Modified";
            begin
                if "File Name" = '' then
        ...
        field(15; Remarks; Text[1020])
        {
            DataClassification = ToBeClassified;
        }
        field(16; "Transfer To"; Text[100])
        {
            TableRelation = "REG-Offices".No;
            trigger OnValidate()
            var
                RO: Record "REG-Offices";
            begin
                //RO.SetRange(No,RO);
            end;
        }
        field(17; "Office Name"; Text[100])
        {
            DataClassification = ToBeClassified;
        }
        field(18; "Folio Number"; Integer)
        {
            DataClassification = ToBeClassified;
        }
        ...
        field(21; Version; code[20])
        {
            DataClassification = ToBeClassified;
        }
        field(22; "Marked for Dist"; Boolean)
        {
            DataClassification = ToBeClassified;
        }
    ...
    keys
    {
        key(Key1; "Table ID", "No.", "Document Type", "Line No.", ID, "Folio Number")
        {
            Clustered = true;
        }
    }
    ...
    begin
        ...
        if Rec.FindLast() then begin
            If "Folio Number" <= 0 then begin
                "Folio Number" := 1;
            end else
                "Folio Number" := "Folio Number" + 1;
        end;
    ...
    local procedure OnBeforeExportAttachment(var DocumentAttachment: Record "REG-Doc Attachment")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeInsertAttachment(var DocumentAttachment: Record "REG-Doc Attachment"; var RecRef: RecordRef)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeSaveAttachment(var DocumentAttachment: Record "REG-Doc Attachment"; var RecRef: RecordRef; FileName: Text; var TempBlob: Codeunit "Temp Blob")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeSaveAttachmentFromStream(var DocumentAttachment: Record "REG-Doc Attachment"; var RecRef: RecordRef; FileName: Text; var DocStream: InStream)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterInitFieldsFromRecRef(var DocumentAttachment: Record "REG-Doc Attachment"; var RecRef: RecordRef)
    begin
    end;
}
