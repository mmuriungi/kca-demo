...
Table 77382 Core_Banking_Details
{
        ...
        field(14; Posted; Boolean)
        {
            ...
            begin
                ...
                if Customer.Find('-') then begin
                    if xRec.Posted = false then
                        if Rec.Posted = true then begin
                            Rec.CalcFields("Receipt Exists");
                            if Rec."Receipt Exists" = true then begin
                                Rec."Posting Status" := Rec."posting status"::Posted;
                            end else begin
                                PostReceiptsFromBuffer(Rec);
                            end;
                            if Rec.Modify then;
                        end else
                            Error('A posted Receipt can not be reopened!');
                end;
            end;
        }
    ...
    procedure PostReceiptsFromBuffer(var CoreBankingDetails: Record Core_Banking_Details)
    var
    ...
    begin
        ...
        if ACAStdPayments.Posted then exit;
