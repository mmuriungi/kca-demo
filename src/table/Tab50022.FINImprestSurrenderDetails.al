table 50022 "FIN-Imprest Surrender Details"
{
    ...
    fields
    {
        ...
        field(7; "Actual Spent"; Decimal)
        {
            ...
            begin
                IF "Actual Spent" > Amount THEN
                    "Over Expenditure" := "Actual Spent" - Amount;
                IF "Actual Spent" <= Amount THEN
                    "Over Expenditure" := 0;
        ...
        field(12; "Cash Receipt No"; Code[20])
        {
            ...
            begin
                ...
                "Date Cash Collected" := ReceiptH.Date;
        ...
        field(29; "Claim Type"; Code[20])
        {
            ...
            trigger OnValidate()
            var
                CTypes: Record "FIN-Receipts and Payment Types";
            begin
                CTypes.Reset();
                CTypes.SetRange("Account Type", CTypes."Account Type"::"G/L Account");
                CTypes.SetRange("Type", CTypes."Type"::Claim);
                CTypes.SetRange("code", Rec."Claim Type");
                if CTypes.find('-') then begin
                    CTypes.TestField("G/L Account");
                    "Claim Account" := CTypes."G/L Account";
                end;
            end;

        }
        field(30; "Claim Account"; Code[20])
        {
            Editable = true;
            NotBlank = true;
            TableRelation = "G/L Account"."No." WHERE("Direct Posting" = filter('Yes'));
        }
        field(31; "Date Cash Collected"; Date)
        {

        }
        field(32; "Acc interest Amount"; decimal)
        {
    ...
    begin
        ...
        Pay.SETRANGE(Pay.No, "Surrender Doc No.");
        //IF Pay.FIND('-') THEN
        //Comment     // IF (Pay.Status = Pay.Status::Posted) OR (Pay.Status = Pay.Status::"Pending Approval")
        // OR (Pay.Status = Pay.Status::Approved) THEN
        //     ERROR('This Document is already Send for Approval/Approved or Posted');
    end;
