table 50019 "FIN-Imprest Header"
{
    ...
    fields
    {
        ...
        field(2; Date; Date)
        {
            ...
            begin
                ...
                if GenLedgerSetup.Get() then begin
                    daysToSurrender := GenLedgerSetup."Surrender Dates";
                    Rec."Expected Date of Surrender" := Date + daysToSurrender;
                end;
        ...
        field(35; Status; Option)
        {
            ...
            Editable = false;
            OptionMembers = Pending,"Pending Approval",Approved,Released,Posted,Cancelled;
        }
        ...
        field(68; "Payment Release Date"; Date)
        {
            ...
            begin
              ...
              */
                ...
                BaseCalendar.SetRange(BaseCalendar.Date, "Payment Release Date");
                if BaseCalendar.Find('-') then begin
                    repeat
                        if BaseCalendar.Nonworking = true then begin
                            if BaseCalendar.Description <> '' then
                                Error('You can not Request Imprest your on a Holiday - ''' + BaseCalendar.Description + '''')
                            else
                                Error('You can notRequest Imprest on a Holiday');
                        end;
                ...
                if ("Surrender Days" <> 0) and ("Payment Release Date" <> 0D) then begin
                    "Expected Date of Surrender" := CalcEndDate("Payment Release Date", "Surrender Days");
        ...
        field(50008; "Payment Schedule No"; Code[30])
        {
            DataClassification = ToBeClassified;
        }
        field(50009; "Expected Date of Surrender"; Date)
        {
            trigger
            OnValidate()
            begin
                "Outstanding Days" := TODAY - "Expected Date of Surrender";
            end;

        }

        field(50010; "Memo No."; Code[20])
        {

        }
        field(50011; "payees bank name"; Text[100])
        {

        }
        field(50012; "payees bank code"; Code[50])
        {

        }
        field(50013; "payees Branch code"; Code[50])
        {

        }
        field(50014; "payees  branch name"; text[100])
        {

        }
        field(50015; "payees bank account"; Code[30])
        {

        }
        field(50016; "Mobile No"; Text[30])
        {

        }
        field(50017; "Job Title"; Text[50])
        {

        }
        field(50018; "Posted To Payroll"; Boolean)
        {

        }
        field(50019; "Transfer By"; Code[20])
        {

        }
        field(50020; "Time Transfered"; Time)
        {

        }
        field(50021; "Date Transfered"; Date)
        {

        }
        field(50022; "Outstanding Days"; Integer)
        {

        }
        field(50023; "Interest Charged"; Boolean)
        {

        }
        field(50024; "Interest Amount"; Decimal)
        {


        }
        field(50025; "Cheque Buffer No"; code[20])
        {
            TableRelation = "FIN-Cheaque Collection Header"."No." where(Closed = filter(false));


        }
        field(50026; "Surrender Days"; Integer)
        {



        }
        field(50027; "Interest Transfered"; Boolean)
        {



        }
        field(50028; "Processpv"; Boolean)
        {



        }
        field(50029; "Voucher No"; code[20])
        {
            TableRelation = "FIN-Payments Header"."No." where(Posted = const(false), "Imprest Voucher" = const(true));



        }
        field(50030; "Pay Using Pv"; Boolean)
        {

        }
        field(50031; "Extend Surrender Date"; Boolean)
        {
    ...
    begin
        ...
        IF "No." = '' THEN BEGIN
            ...
            "Surrender Days" := GenLedgerSetup."Surrender Dates";
            //END
        END;
    ...
    procedure CalcEndDate(SDate: Date; SurDays: Integer) SurEndDate: Date
    var
        EndSur: Boolean;
        DayCount: Integer;
        GenLedgerSetup: Record "Cash Office Setup";
    begin
        ...
        SDate := SDate;//-1;
        EndSur := false;
        while EndSur = false do begin
            if not DetermineIfIsNonWorking(SDate, GenLedgerSetup) then
                DayCount := DayCount + 1;
            SDate := SDate + 1;
            if DayCount = SurDays then
                EndSur := true;
        end;
        SurEndDate := SDate - 1;

        while DetermineIfIsNonWorking(SurEndDate, GenLedgerSetup) = true do begin
            SurEndDate := SurEndDate + 1;
        end;
    end;

    procedure DetermineIfIsNonWorking(var bcDate: Date; var GenLedgerSetup: Record "Cash Office Setup") ItsNonWorking: Boolean
    var
        dates: Record Date;
    begin
        Clear(ItsNonWorking);
        ...
        if ItsNonWorking = false then begin
            ...
            if dates.Find('-') then begin
                ...
                if dates."Period Name" = 'Sunday' then begin
                    ...
                    if GenLedgerSetup."Inclusive of Sunday" = false then ItsNonWorking := true;
                end else
                    if dates."Period Name" = 'Saturday' then begin
                        //check if Leave includes sato
                        if GenLedgerSetup."Inclusive of Saturday" = false then ItsNonWorking := true;
                    end;
            end;
        end;
    end;

    procedure DetermineIfIncludesNonWorking(): Boolean
    begin
        if GenLedgerSetup.Get() then begin
            if GenLedgerSetup."Inclusive of Non Working Days" = true then
                exit(true);
        end;
    end;
}
