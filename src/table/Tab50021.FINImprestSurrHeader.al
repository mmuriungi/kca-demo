table 50021 "FIN-Imprest Surr. Header"
{
    DrillDownPageId = "FIN-Imprest Accounting";
    LookupPageId = "FIN-Imprest Accounting";
    ...
    fields
    {
                ...
                IF PayLine.FIND('-') THEN /*Copy the lines to the line table in the database*/
                  BEGIN
                    REPEAT
                        ...
                        if PayHeader."Expected Date of Surrender" < Today then //begin
                            OutstandingDays := (Today - PayHeader."Expected Date of Surrender");
                        GenLedgerSetup.Get();
                        interestAmount := GenLedgerSetup."Interest %";
                        ImpSurrLine."Acc interest Amount" := OutstandingDays * (interestAmount / 100);
        ...
        field(50007; "Claim Type"; Code[30])
        {
            CalcFormula = lookup("FIN-Imprest Surrender Details"."Claim Type" WHERE("Surrender Doc No." = FIELD(No)));
            FieldClass = FlowField;
        }
        field(50008; "Claim Account"; Code[20])
        {
            CalcFormula = lookup("FIN-Imprest Surrender Details"."Claim Account" WHERE("Surrender Doc No." = FIELD(No)));
            FieldClass = FlowField;
        }
        field(50009; "Claim No"; Code[20])
        {
            // CalcFormula = lookup("FIN-Imprest Surrender Details"."Claim Account" WHERE("Surrender Doc No." = FIELD(No)));
            FieldClass = FlowField;
            CalcFormula = lookup("FIN-Staff Claims Header"."No." where("Imprest No." = field("Imprest Issue Doc. No")));
        }
        field(50010; "Interest Recovered"; Boolean)
        {
            // CalcFormula = lookup("FIN-Imprest Surrender Details"."Claim Account" WHERE("Surrender Doc No." = FIELD(No)));

        }
        field(50013; purpose; text[250])
        {

        }
        field(50011; "Created Via Recovery"; Boolean)
        {
            // CalcFormula = lookup("FIN-Imprest Surrender Details"."Claim Account" WHERE("Surrender Doc No." = FIELD(No)));
            trigger OnValidate()
            begin
                ...
                ImpSurrLine.SetRange("Account No:", ImpSurrLine."Account No:");
                IF ImpSurrLine.FIND('-') THEN /*Copy the lines to the line table in the database*/
                  BEGIN
                    REPEAT
                        //ImpSurrLine.Init();
                        ImpSurrLine."Actual Spent" := ImpSurrLine.Amount;
                        ImpSurrLine.Modify()
        ...
        field(50012; "post via Recovery"; Boolean)
            ...
            begin
                ...
                IF ImprestDetails.FIND('-') THEN BEGIN
                    REPEAT
                        ...
                        GenJnlLine.Description := 'Imprest Surrendered by staff By ' + ':' + ImprestDetails."Imprest Holder";
                        ...
                        if ImprestDetails."Acc interest Amount" <> 0 then begin
                            GenledSetup.get;
                            GenledSetup.TestField("Interest G/l Acc");
                            InterestGLAccount := GenledSetup."Interest G/l Acc";
