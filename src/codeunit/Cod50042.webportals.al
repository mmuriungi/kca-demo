...
#pragma warning restore AL0667
  ...
  + '::' + FORMAT(CurrentSem."Mark entry Dealine") + '::' + FORMAT(CurrentSem."Lock Lecturer Editing") + '::' + FORMAT(CurrentSem.AllowDeanEditing) + '::' + FORMAT(CurrentSem."Unit Registration Deadline");
    ...
    begin
        ...
        IF CourseRegistration.FIND('-') THEN BEGIN
            REPORT.SAVEASPDF(Report::"Provisional College Transcript", filename, CourseRegistration);
        END;
    end;
  ...
  KUCCPSRaw.Email + '::' + KUCCPSRaw.Phone + '::' + GetSchool(KUCCPSRaw.Prog) + '::' + GetProgram(KUCCPSRaw.Prog) + '::' + KUCCPSRaw.Prog;
    ...
    begin
        ...
        IF AplicFormHeader.FIND('-') THEN BEGIN
            REPORT.SAVEASPDF(Report::"Official Admission Letter -SSP", filename, AplicFormHeader);
        END;
    end;
    ...
    procedure UpdateKUCCPSProfile(username: Text; Genderz: Integer; Phonez: Code[20]; Boxz: Code[50]; Codesz: Code[20]; Townz: Code[40]; Emailz: Text; Countyz: Code[50]; DateofBirth: Date; IDNumber: Text; PhysicalImpairments: Integer; PhysicalImpairmentsDetails: Text; Ethnic: Code[50]; Nationalityz: Code[50])
    begin
        KUCCPSRaw.RESET;
        KUCCPSRaw.SETRANGE(KUCCPSRaw.Index, username);
        IF KUCCPSRaw.FIND('-') THEN BEGIN
    ...
    begin
        ...
        IF StudentsTransfer.FIND('-') THEN BEGIN
            REPORT.SAVEASPDF(Report::"Aca-Students Transfer Report", filename, StudentsTransfer);
        END;
    end;
