page 50044 "FIN-Travel Advance Acc. UP"
{
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                field("Created Via Recovery"; Rec."Created Via Recovery")
                {
                    ApplicationArea = all;

                }
                field("post via Recovery"; Rec."post via Recovery")
                {
                    ApplicationArea = all;
    ...
    actions
    {
        area(processing)
        {
            group(Functions)
            {
                ...
                action(sendApproval)
                {
                    ...
                    begin
                        ...
                        //     REPEAT
                        //         IF (ImprestDetails."Cash Receipt Amount" + ImprestDetails."Actual Spent" + ImprestDetails."Over Expenditure") <> ImprestDetails.Amount THEN
            ...
            action(Post)
            {
                ...
                begin
                    ...
                    IF ImprestReq.FIND('-') THEN BEGIN
                        ...
                        ImprestReq.MODIFY;
                    END;
                    Rec.CalcFields("Over Expenditure");
                    if Rec."Over Expenditure" > 0 then
                        GenerateClaim();
            ...
            {
                Caption = 'Recover Interest';
                //Visible = false;

                trigger OnAction()
                begin
                    // CheckImprestRequiredItems;
                    Rec.SetFilter("Interest Recovered", '%1', false);
                    // Rec.SetFilter("Expected Date of Surrender", '<%1', TODAY);
                    IF CONFIRM('Recover Interest', TRUE) = FALSE THEN EXIT;
    ...
    var
        CHead: Record "FIN-Staff Claims Header";
        CLines: Record "FIN-Staff Claim Lines";
        glAccount: Record "G/L Account";
    ...
    begin
        ...
        if Rec."Over Expenditure" > 0 then begin
            ...
            Nextno := NoSeriesMgt.GetNextNo(GenLedgerSetup."Staff Claim No", today, true);
            CHead.Init();
            CHead."No." := Nextno;
            Rec."Claim No" := CHead."No.";
            CHead."Date" := Rec."Surrender Date";
            CHead."Global Dimension 1 Code" := Rec."Global Dimension 1 Code";
            CHead."Shortcut Dimension 2 Code" := Rec."Shortcut Dimension 2 Code";
            CHead."Imprest No." := Rec."Imprest Issue Doc. No";
            CHead."Surrender No." := Rec.No;
            CHead."Account Type" := CHead."Account Type"::Customer;
            CHead."Account No." := Rec."Account No.";
            Cust.Reset();
            Cust.SetRange("No.", Rec."Account No.");
            if Cust.FindFirst() then
                CHead.Payee := Cust.Name;
            CHead.Purpose := 'Refund of the over Expenditure Incurred';
            imprestHeader.Reset();
            imprestHeader.SetRange("No.", Rec."Imprest Issue Doc. No");
            ...
            CLines.Init();
            CLines.No := CHead."No.";
            CLines."Advance Type" := Rec."Claim Type";
            CLines."Account No:" := Rec."Claim Account";
            glAccount.Reset();
            glAccount.SetRange("No.", Rec."Claim Account");
            if glAccount.FindFirst then
                CLines."Account Name" := glAccount.Name;
            CLines.Amount := Rec."Over Expenditure";

            CLines."Expenditure Date" := Rec."Imprest Issue Date";
            CLines.Purpose := 'Refund on Over Expenditure Incurred';
            CLines.Insert();
            sendClaimApproval.OnSendClaimforApproval(CHead);
            Message('Claim No: ' + format(CHead."No.") + ' Generated Successifully');
        end;
        rec."Claim No" := Nextno;
        ...
        IF FINImprestSurrLines.FIND('-') THEN BEGIN
            ...
            BEGIN
                ...
                IF (FINImprestSurrLines."Actual Spent" > 0) THEN BEGIN
                    // Commit Budget Here
                    PostBudgetEnties.ExpenseBudget(FINImprestSurrLines."Account No:", Rec."Surrender Date", REc."Global Dimension 1 Code", '',
                    FINImprestSurrLines."Actual Spent", FINImprestSurrLines."Account Name", USERID, TODAY, 'IMPREST', Rec."Imprest Issue Doc. No" + FINImprestSurrLines."Account No:", '', Rec.Payee);
                END;
            END;
    ...
    begin
        ...
        IF payrollperiod.FIND('-') THEN begin
            ...
            IF ImprestDetails.FIND('-') THEN Begin
                ...
                PrlEmployeeTransactions.SetRange("Employee Code", Rec."Account No.");
                PrlEmployeeTransactions.SetRange("Payroll Period", PayrollPeriod."Date Opened");
                if not PrlEmployeeTransactions.FindFirst() then begin
                    Repeat


                        PrlEmployeeTransactions.Init();
                        PrlEmployeeTransactions."Employee Code" := Rec."Account No.";
                        PrlEmployeeTransactions."Transaction Code" := 'D-0002';

                        PrlEmployeeTransactions.Amount := ImprestDetails."Acc interest Amount";
                        PrlEmployeeTransactions."Payroll Period" := PayrollPeriod."Date Opened";
                        PrlEmployeeTransactions."Period Year" := PayrollPeriod."Period Year";
                        PrlEmployeeTransactions."Period Month" := PayrollPeriod."Period Month";
                        PrlEmployeeTransactions."Recurance Index" := 1;
                        PrlEmployeeTransactions.Insert();
                        PeriodTransactioncodes.reset;
                        PeriodTransactioncodes.SetRange("Transaction Code", PrlEmployeeTransactions."Transaction Code");
                        if PeriodTransactioncodes.FindFirst() then begin
                            PrlEmployeeTransactions."Transaction Name" := PeriodTransactioncodes."Transaction Name";
                        ...
                        ImprestReq.SETRANGE(ImprestReq."No.", Rec."Imprest Issue Doc. No");
                        IF ImprestReq.FIND('-') THEN BEGIN
                            ImprestReq."Interest Transfered" := true;
                            ImprestReq.MODIFY;
                        END;
                        Rec."Interest Recovered" := TRUE;
                        REC.Modify()


                    until PrlEmployeeTransactions.Next() = 0;
                    Message('Interest Transfered To Payroll Sucessfull');
                    CurrPage.Close();
