page 51495 "HMS-Treatment Lab History"
{
    PageType = Document;
    Editable = true;
    InsertAllowed = true;
    ...
    actions
    {
        area(processing)
        {
            action("Update Lab Entries")
            {
                Promoted = true;
                PromotedCategory = Process;
                ApplicationArea = All;
                Visible = false;
                RunObject = Report "Update Lab Details";
            }
            /* action("Update Lab Patient")
            {
                Promoted = true;
                PromotedCategory = Process;
                ApplicationArea = All;
                RunObject = Report "Update Lab Patient No";
            } */
            action("Send to lab")
            {
                Caption = 'Send To Lab';
            ...
            action("Send to Pharmacy")
            {
                ...
                trigger OnAction()
                begin
                    //  Confirm('Send to the Pharmacist ? ', true):False Then ERROR('Cancelled buy user');
