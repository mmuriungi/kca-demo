...
/// </summary>
page 51164 "ACA-KUCCPS Student Reg"
{
    ...
    layout
    {
        area(content)
        {
            repeater(Group)
            {
                ...
                field("Student E-mail"; Rec."Student E-mail")
                {
                    Caption = 'Personal E-mail';
                    ApplicationArea = All;
                }
    ...
    actions
    {
        ...
        area(processing)
        {
            action(GenerateAdmissionNo)
            {
                ...
                begin
                    ...
                    IF AdminSetup.FIND('-') THEN BEGIN
                        IF ((Rec."Settlement Type" = 'KUCCPS') OR (Rec."Settlement Type" = 'GOK') OR (Rec."Settlement Type" = 'JAB')) THEN
                            settlementPrefix := AdminSetup."JAB Prefix"
                        ELSE
                            IF ((Rec."Settlement Type" = 'PSSP') OR (Rec."Settlement Type" = 'SSP') OR (Rec."Settlement Type" = 'SELFSPONSORED')) THEN
                ...
                begin
                    if Rec."Programme Level" = Rec."Programme Level"::certificate then begin
                        ...
                        if Rec."Programme Level" = Rec."Programme Level"::Diploma then begin
                            ...
                            if ((Rec."Programme Level" = Rec."Programme Level"::Undergraduate)) then begin

                                admletter.Reset();
                                admletter.SetRange("Application No.", Rec."Application No.");
                                Report.Run(66679, true, true, admletter);
                            end else
                                if Rec."Programme Level" = Rec."Programme Level"::Postgraduate then begin
            ...
            action("Extract Data")
            {
                ...
                Promoted = true;
                PromotedCategory = Process;
                RunObject = xmlport "Admissions Data";
            }
            action("Updatelevels2")
            {
                ApplicationArea = All;
                Promoted = true;
                PromotedCategory = Process;
    ...
    var
        ...
        kuccp: record "KUCCPS Imports";
        aPP: record "ACA-Applic. Form Header";

    /// <summary>
    /// GetCountry.
    /// </summary>
    /// <param name="CountryCode">VAR Code[20].</param>
    /// <returns>Return variable CountryName of type Text[100].</returns>
...
#pragma warning restore AL0523
    ...
    begin
        aPP.Reset();
        aPP.SetRange("Settlement Type", 'GSS');
        IF aPP.Find('-') then begin
            repeat
                kuccp.Reset();
                kuccp.SetRange(Index, aPP."Index Number");
                if kuccp.Find('-') then begin
                    app.gender := kuccp.Gender;
                    app."Admission No" := kuccp.Admin;
                    app.Modify();
                end;
            until aPP.Next() = 0;
  ...
  Cust."Date Of Birth":=Admissions."Date Of Birth";
    ...
    begin
        apps.Reset();
        apps.SetRange("Settlement Type", '4YR-HEF');
        if apps.Find('-') then begin
            repeat
                kuccps.Reset();
                kuccps.SetRange(Index, apps."Index Number");
                if kuccps.Find('-') then begin
                    apps.Campus := kuccps."Campus Location";
                    apps.Modify();
                end;
            until apps.Next() = 0;
        end;
    ...
    begin
        ...
        if apps.Find('-') then begin
            repeat
                prog.Reset();
                prog.SetRange(code, apps."First Degree Choice");
                if prog.Find('-') then begin
                    apps."Programme Level" := prog.Levels;
                    apps.Modify();
                end;
            until apps.Next() = 0;
        end;
