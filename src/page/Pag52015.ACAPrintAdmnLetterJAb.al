page 52015 "ACA-Print Admn Letter JAb"
{
    DeleteAllowed = false;
    PageType = List;
                            ...
                            REPEAT
                                ...
                                IF SendToPrinter = TRUE THEN BEGIN
                                    IF ((progs.Category <> progs.Category::Diploma) AND (progs.Category <> progs.Category::Certificate)) THEN
                                        REPORT.RUN(Report::"Official Admission LetterJAb", FALSE, SendToPrinter, Apps)
                                    ELSE
                                        REPORT.RUN(Report::"Official Admission LetterJAb", FALSE, SendToPrinter, Apps);
                                END
                                ELSE BEGIN
                                    IF ((progs.Category <> progs.Category::Diploma) AND (progs.Category <> progs.Category::Certificate)) THEN
                                        REPORT.RUN(Report::"Official Admission LetterJAb", TRUE, SendToPrinter, Apps)
                                    ELSE
                                        REPORT.RUN(Report::"Official Admission LetterJAb", TRUE, SendToPrinter, Apps);
                                END;
                            UNTIL Apps.NEXT = 0;
