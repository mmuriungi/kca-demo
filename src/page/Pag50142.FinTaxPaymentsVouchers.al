page 50142 "Fin-Tax Payments Vouchers"
{
    ApplicationArea = All;
    Caption = 'Fin-Tax Payments Vouchers';
    PageType = List;
    UsageCategory = Administration;
    CardPageID = "Fin-Tax Payment Voucher";
    Editable = false;
    SourceTable = "FIN-Payments Header";
    SourceTableView = WHERE(Posted = filter(false),
                            "Payment Type" = CONST(Normal),
                            Status = FILTER(Pending | "Pending Approval" | Approved), "Tax Voucher" = filter(true));
        ...
        area(processing)
        {
            group("&Functions")
            {
                ...
                action(sendApproval)
                {
                    ...
                    begin
                        CommitBudget;
                        IF ApprovalMgt.IsPVSEnabled(Rec) THEN
                            ApprovalMgt.OnSendPVSforApproval(Rec);
                    end;
                }
