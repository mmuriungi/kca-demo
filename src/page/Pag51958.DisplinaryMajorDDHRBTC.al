page 51958 "Displinary Major DDHR B_TC"
{
    PageType = Card;
    SourceTable = "Displinary Cases";
    SourceTableView = where("Case Status" = filter(DDHR_Board_T));
    ...
    actions
    {
        area(Processing)
        {
            ...
            action(CommitteeMembers1)
            {
                Caption = 'JSAC Members';
                ApplicationArea = All;
                Promoted = true;
                PromotedCategory = Process;
            ...
            action(Recommendations3)
            {
                Caption = 'Board Recommendations';
                ApplicationArea = All;
                PromotedCategory = Process;
                Promoted = true;
                Image = Reconcile;
                RunObject = Page "Displinary Committess REC";
                RunPageLink = "Case No." = field("No."), Committe = field("3rd Committee Identified");
            }
            action(Suspension)
            {
                Caption = 'SUspension';
                ...
                trigger OnAction()
                begin
                    Rec."Case Status" := Rec."Case Status"::Suspension;
                end;
            }
        }
