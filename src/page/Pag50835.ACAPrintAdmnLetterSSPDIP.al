page 50835 "ACA-Print Admn Letter SSP/DIP"
{
    DeleteAllowed = false;
    PageType = List;
                            ...
                            REPEAT
                                ...
                                IF SendToPrinter = TRUE THEN BEGIN
                                    IF ((progs.Category <> progs.Category::Diploma) AND (progs.Category <> progs.Category::Certificate)) THEN
                                        REPORT.RUN(Report::"Official Admission Letter -SSP", FALSE, SendToPrinter, Apps)
                                    ELSE
                                        REPORT.RUN(Report::"Official Admission Letter -SSP", FALSE, SendToPrinter, Apps);
                                END
                                ELSE BEGIN
                                    IF ((progs.Category <> progs.Category::Diploma) AND (progs.Category <> progs.Category::Certificate)) THEN
                                        REPORT.RUN(Report::"Official Admission Letter -SSP", TRUE, SendToPrinter, Apps)
                                    ELSE
                                        REPORT.RUN(Report::"Official Admission Letter -SSP", TRUE, SendToPrinter, Apps);
                                END;
                            UNTIL Apps.NEXT = 0;
