...
/// </summary>
page 50928 "ACA-Std Billing List"
{
    ...
    procedure OpenCustomerLedgerEntries1()
    var
    ...
    begin
        IsHandled := false;
        OnBeforeOpenCustomerLedgerEntries(Rec, DetailedCustLedgEntry, IsHandled);
        if IsHandled then
            exit;
        ...
        if (Rec.GetFilter("Date Filter") <> '') then begin
            Rec.CopyFilter("Date Filter", DetailedCustLedgEntry."Initial Entry Due Date");
            DetailedCustLedgEntry.SetFilter("Posting Date", '<=%1', Rec.GetRangeMax("Date Filter"));
        end;
        Rec.CopyFilter("Currency Filter", DetailedCustLedgEntry."Currency Code");
        DrillDownOnEntries(DetailedCustLedgEntry);
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeOpenCustomerLedgerEntries(var Customer: Record Customer; var DetailedCustLedgEntry: Record "Detailed Cust. Ledg. Entry"; var IsHandled: Boolean)
    begin
    end;
