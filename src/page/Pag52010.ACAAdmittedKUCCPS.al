/// <summary>
/// Page ACA-Admitted KUCCPS (ID 70086).
/// </summary>
page 52010 "ACA-Admitted KUCCPS"
{
    ...
    SourceTableView = WHERE("Settlement Type" = const('GSS'), Admitted = const(True));
...
#pragma warning restore AL0523
    ...
    begin
        Apps.Reset();
        Apps.SetRange("Documents Verified", true);
        if Apps.Find('-') then begin
            repeat
                Admissions.Reset();
                Admissions.SetRange("Application No.", Apps."Application No.");
                if Admissions.Find('-') then begin
                    Apps.Status := Apps.Status::Admitted;
                    Apps.Modify();
                end;
            until Apps.Next() = 0;
    ...
    procedure updateREg2()
    begin
        Apps.Reset();
        Apps.SetRange(Status, Apps.Status::Admitted);
        if Apps.Find('-') then begin
            repeat
                courseReg.Reset();
                courseReg.SetRange("Student No.", Apps."Admission No");
                if courseReg.find('-') then begin
                    Rename(courseReg.Programmes)
                end;
    ...
    /// <returns>Return variable FacultyName of type Text[100].</returns>
    /// 
    /// 
    ...
    /// <returns>Return variable AgeText of type Text[100].</returns>
    // procedure GetAge(var StartDate: Date) AgeText: Text[100]
    // var
    //     HrDates: Codeunit "50126";
    // begin
