page 50968 "Repeated Units Registration"
{
    PageType = List;
    SourceTable = "ACA-Student Units";
    SourceTableView = where(Occurances = filter(> 1));
    DeleteAllowed = false;
    InsertAllowed = false;
    ModifyAllowed = false;
    ...
    actions
    {
        area(processing)
        {
            action(DeleteReapetitions)
            {
                ...
                trigger OnAction()
                var
                    OccuranceTimes: Integer;
                begin
                    if confirm('Delete', false) = false then Error('Cancelled!');
                    Clear(OccuranceTimes);
                    StudUnits.RESET;
                    StudUnits.SETRANGE(StudUnits."Student No.", Rec."Student No.");
                    StudUnits.SETRANGE(StudUnits.Unit, Rec.Unit);
                    IF StudUnits.FIND('-') THEN BEGIN
                        REPEAT
                            StudUnits.CalcFields(Occurances);
                            OccuranceTimes := StudUnits.Occurances;
                            if OccuranceTimes > 1 then
                                IF StudUnits.Delete(true) = TRUE THEN BEGIN
                                END;
                        UNTIL OccuranceTimes = 1;
                        Message('Deleted!');
                    END;
                end;
            }
