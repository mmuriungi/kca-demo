page 50140 "Fin-Tax Payment Voucher"
{
    Caption = 'Fin-Tax Payment Voucher';
    PageType = Card;
    ...
    SourceTableView = WHERE(Posted = filter(false), Status = FILTER(<> Cancelled), "Tax Voucher" = filter(true));
    ...
    layout
    {
        area(content)
        {
            ...
            part(PVLines; "Fin-Tax Payment Lines ")
            {
                ApplicationArea = All;
    ...
    var
        TaxPaymentLedgers: Record "FIN-Tax Payment Ledgers";
    ...
    begin
        ...
        IF PayLine.FIND('-') THEN BEGIN
            ...
            END;
            TaxPaymentLedgers.Reset;
            TaxPaymentLedgers.SetRange("Tax Pv No", Rec."No.");
            if not TaxPaymentLedgers.Find('-') Then Begin
                TaxPaymentLedgers.Init();
                TaxPaymentLedgers."Entry No" := GetLastTaxLedgerEntryNo() + 1;
                TaxPaymentLedgers."Tax Pv No" := Rec."No.";
                TaxPaymentLedgers."Vendor  Pv No" := PayLine."Vendor pv";
                TaxPaymentLedgers."Paid Amount" := PayLine.Amount;
                TaxPaymentLedgers."Posting Date" := Payment."Payment Release Date";
                TaxPaymentLedgers."Tax Type" := PayLine."Tax Type";
                TaxPaymentLedgers."Vendo No" := PayLine."Tax Account No";
                TaxPaymentLedgers.Description := Payment."Payment Narration";

                TaxPaymentLedgers.Insert()
    ...
    procedure GetLastTaxLedgerEntryNo(): Integer;
    var
        mrktLedger: Record "FIN-Tax Payment Ledgers";
    begin
        mrktLedger.Reset();
