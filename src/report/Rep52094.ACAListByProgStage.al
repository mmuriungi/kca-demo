report 52094 "ACA-List By Prog Stage"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/ACA-List By Prog Stage.rdl';
    ...
    dataset
    {
        dataitem("ACA-Course Registration"; "ACA-Course Registration")
        {
            DataItemTableView = SORTING(Stage)
                                ORDER(Ascending)
                                WHERE(Programmes = FILTER(<> ''));
            RequestFilterFields = "Campus Code";
            ...
            column(semz; FORMAT(Semesters))
            {
            }
            column(Totalbilled; CourseReg."Total Billed")
            {
            }
            column(Semester; CourseReg.Semester)
            {
            }
