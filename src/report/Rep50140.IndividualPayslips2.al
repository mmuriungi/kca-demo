report 50140 "Individual Payslips 2"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Individual Payslips.rdl';

    dataset
    {
        dataitem(salCard; "PRL-Salary Card")
        {
            DataItemTableView = SORTING("Employee Code", "Payroll Period") ORDER(Ascending) WHERE("Basic Pay" = FILTER(<> 0));
            ...
            begin
                ...
                PeriodTrans.Reset;
                PeriodTrans.SetRange(PeriodTrans."Employee Code", salCard."Employee Code");
                ...
                emp1.Reset;
                emp1.SetRange(emp1."No.", salCard."Employee Code");
                ...
                *****************************************************************************************************/
                ...
                Clear(daysToRetir);
                if objEmp."Salary Category" <> '' then begin
                    HRMEmployeeCategories.Reset;
                    HRMEmployeeCategories.SetRange(Code, objEmp."Salary Category");
                    if HRMEmployeeCategories.Find('-') then begin
                        //Compute Retirement Period
                        if HRMEmployeeCategories."Retirement Age" <> 0 then begin
                            if HRMSetup.Get then begin
                                //IF HRMSetup."Days To Retirement"<>'' THEN BEGIN
                                daysToRetir := HRDates.DetermineAge(Today, CalcDate(Format(HRMEmployeeCategories."Retirement Age") + 'Y', objEmp."Date Of Birth"));
                            end;
                            //END;
                        end;
                    end;
                ...
                *******************************************************************************/
                ...
                Addr[ColumnNo, 11] := 'Retirement:                ' + daysToRetir;
                    ...
                    repeat
                        ...
                        if strGrpText <> PeriodTrans."Group Text" then begin
                            ...
                            if (PeriodTrans."Group Text" <> 'BASIC SALARY') and (PeriodTrans."Group Text" <> 'GROSS PAY')
                                        and (PeriodTrans."Group Text" <> 'NET PAY') then begin
                                if PeriodTrans.Balance > 0 then begin
                                    //PeriodTrans.CalcFields(Balance);
                                    Index := Index + 1;
                                    ...
                                    if PeriodTrans."Transaction Name" = 'Provident (Self)' then begin
                                        ...
                                        Evaluate(TransAmt[ColumnNo, Index], Format(Round(salCard."Basic Pay" * 0.1 - 200, 2, '=')));
                                        //  Index:=Index+1;
                                    ...
                                    if PeriodTrans."Transaction Name" = 'Provident (Self)' then begin
                                        Trans[ColumnNo, Index] := 'Provident Employer';
                                        Evaluate(TransAmt[ColumnNo, Index], Format(Round(salCard."Basic Pay" * 1 - 200, 2, '=')));
            ...
            begin
                if salCard.GetFilter("Payroll Period") <> '' then begin
                    Evaluate(PeriodFilter, (salCard.GetFilter("Payroll Period")));
                end;
    ...
    var
        ...
        HRMEmployeeCategories: Record "HRM-Employee Categories";
        daysToRetir: Text[250];
