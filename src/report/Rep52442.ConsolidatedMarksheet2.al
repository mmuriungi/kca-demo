/// <summary>
/// Report Consolidated Marksheet 2 (ID 51698).
/// </summary>
report 52442 "Consolidated Marksheet 2"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Consolidated Marksheet 2.rdl';
...
#pragma warning restore AL0667
            ...
            column(Course_Registration__GETFILTER_Options_; GETFILTER(Options))
            {
            }
            ...
            column(Course_Registration__Course_Registration___Student_No__; "Student No.")
            {
            }
            ...
            begin
                ...
                UnitsR.SETFILTER(UnitsR."Reg. ID Filter", "Reg. Transacton ID");
                ...
                IF StudUnits.FIND('-') THEN BEGIN
                    REPEAT
                        IF GetGradeStatus(StudUnits."Final Score", Programmes, StudUnits.Unit) = TRUE THEN
                            StudUnits."Result Status" := 'FAIL'
                        ELSE
                            StudUnits."Result Status" := 'PASS';
                ...
                IF DSummary = FALSE THEN BEGIN
                    ...
                    ColumnV[i] := FORMAT("Cum Units Done");
                    ...
                    //IF UTaken > 0 THEN
                    ColumnV[i] := FORMAT("Cum Units Passed");
                    AvScore[i] := AvScore[i] + "Cum Units Passed";


                    i := i + 1;
                    ColumnH[i] := 'Papers Failed';
                    ColumnV[i] := FORMAT("Cum Units Done" - "Cum Units Passed");
                    AvScore[i] := AvScore[i] + "Cum Units Failed";

                    i := i + 1;
                    ColumnH[i] := 'CF% Failed';
                    IF ("Cum Units Passed" <> 0) AND ("Cum Units Done" <> 0) THEN
                        ColumnV[i] := FORMAT(ROUND((("Cum Units Done" - "Cum Units Passed") /
                       "Cum Units Done") * 100, 1));
                    ...
                    IF ("CF Total Score" <> 0) AND ("CF Count" <> 0) THEN BEGIN
                        ColumnV[i] := FORMAT(ROUND("CF Total Score" / "CF Count", 1, '='));
                        AvScore[i] := AvScore[i] + ROUND("CF Total Score" / "CF Count", 1, '=');
                    ...
                    // Current Average
                    i := i + 1;
                    MaxYear := GETRANGEMAX("Stage Filter");
                    MaxSem := GETRANGEMAX("Semester Filter");

                    ColumnH[i] := 'Cur Average';
                    CReg.RESET;
                    CReg.SETRANGE(CReg."Student No.", "Student No.");
                    CReg.SETFILTER(CReg.Programmes, Programmes);
                    ...
                    DefUnit := 'DEF';
                    ColumnH[i] := 'Grade';
                    IF ("CF Total Score" <> 0) AND ("CF Count" <> 0) THEN
                        ColumnV[i] := GetGrade(ROUND("CF Total Score" / "CF Count", 0.5, '='), DefUnit);

                    i := i + 1;

                    ColumnH[i] := 'Rmk';
                    ColumnV[i] := "Exam Status";
                    ...
                    IF ("Cum Units Done" <> 0) AND ("Cum Units Passed" <> 0)
                    AND ("Cum Units Failed" = 0) THEN
                        IF "Cum Units Done" = "Cum Units Passed" THEN BEGIN
                    ...
                    IF "Cum Units Failed" > 7 THEN BEGIN
                    ...
                    IF ("Cum Units Failed" < 8) AND ("Cum Units Failed" > 5) THEN BEGIN
            ...
            begin
                ...
                IF Sem.GET(GETFILTER(Semester)) THEN
                ...
                IF CReg.FIND('-') THEN BEGIN
                    REPEAT
                        ...
                        IF (CReg.Options <> '') THEN BEGIN
                            ...
                            OptionsComb.SETRANGE(OptionsComb.Programme, CReg.Programmes);
    ...
    var
        ...
        UnitsRRR: Record "ACA-Units/Subjects";
        ...
        DontCont: Boolean;
        CAve: Decimal;
        Intake: Record "ACA-Course Registration";
        ShowCount: Integer;
        ...
        TRepeat: Integer;
        TDiscount: Integer;
        ResultStatus: Record "ACA-Results Status";
        SMM: array[20] of Text[100];
        DefUnit: Code[20];
        OptionsComb: Record "ACA-Units Options Combination";
